...
class App {
  private:
    ...
    void play_card(std::optional<Card> card) {
      ...
      String path =
        card.has_value()
          ? audio_db_path + "/" + card.value().file
          : audio_db_path + "/" + config.value().unknownCardSfx;

      if (!card.has_value()) {
        debug_print("Audio file not found: %s", path.c_str());
        String fallbackAudioPath = config.value().audiodbPath + "/" + config.value().unknownCardSfx;
        audio.connecttoFS(SD, fallbackAudioPath.c_str());
        return;
      }

      if (!SD.exists(path)) {
        debug_print("Audio file not found: %s", path.c_str());
        audio.connecttoFS(SD, config.value().unknownCardSfx.c_str());
        return;
      }
